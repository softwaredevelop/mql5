//+------------------------------------------------------------------+
//|                                             GScore_Calculator.mqh|
//|      Engine for G-Score (Ehlers Gaussian Filter Z-Score).         |
//|      Strictly O(1) Incremental Optimized.                        |
//|                                        Copyright 2026, xxxxxxxx  |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // Universal G-Score calculator with Heikin Ashi support

#ifndef GSCORE_CALCULATOR_MQH
#define GSCORE_CALCULATOR_MQH

#include <MyIncludes\Gaussian_Filter_Calculator.mqh>

//+==================================================================+
//|             CLASS: CGScoreCalculator                             |
//+==================================================================+
class CGScoreCalculator
  {
private:
   ...
   CGaussianFilterCalculator *m_gaussian_calc;
...
public:
   ...
   bool              Init(int period, bool use_ha);
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CGScoreCalculator::CGScoreCalculator() : m_period(20), m_use_ha(false), m_gaussian_calc(NULL) {}

//+------------------------------------------------------------------+
//| Destructor                                                       |
//+------------------------------------------------------------------+
CGScoreCalculator::~CGScoreCalculator()
  {
   if(CheckPointer(m_gaussian_calc) != POINTER_INVALID)
      delete m_gaussian_calc;
  }

//+------------------------------------------------------------------+
//| Init                                                             |
//+------------------------------------------------------------------+
bool CGScoreCalculator::Init(int period, bool use_ha)
  {
   m_period = (period < 2) ? 2 : period;
   m_use_ha = use_ha;

   if(m_use_ha)
      m_gaussian_calc = new CGaussianFilterCalculator_HA();
   else
      m_gaussian_calc = new CGaussianFilterCalculator();

   if(CheckPointer(m_gaussian_calc) == POINTER_INVALID)
      return false;

   return m_gaussian_calc.Init(m_period, SOURCE_PRICE);
  }

//+------------------------------------------------------------------+
//| Calculate                                                        |
//+------------------------------------------------------------------+
void CGScoreCalculator::Calculate(int rates_total, int prev_calculated, ENUM_APPLIED_PRICE price_type,
                                  const double &open[], const double &high[], const double &low[], const double &close[],
                                  double &out_gscore[])
  {
...
// 2. Calculate underlying Gaussian Filter baseline
   m_gaussian_calc.Calculate(rates_total, prev_calculated, price_type, open, high, low, close, m_smooth_buf);
   ...
   for(int i = start; i < rates_total; i++)
     {
      ...
      if(std_dev > 1.0e-9)
         out_gscore[i] = (p - current_smooth) / std_dev;
      else
         out_gscore[i] = 0.0;
     }
  }
...
//| PreparePriceSeries                                               |
//+------------------------------------------------------------------+
bool CGScoreCalculator::PreparePriceSeries(int rates_total, int start_index, ENUM_APPLIED_PRICE price_type, const double &open[], const double &high[], const double &low[], const double &close[])
  {

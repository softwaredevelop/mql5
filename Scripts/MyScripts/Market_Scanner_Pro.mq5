...
// =================================================================
// ADVANCED ABSORPTION LOGIC v3 (Wyckoff + Velocity Confirmation)
// =================================================================
   ...
   if(idx_cl_mid >= 0 && mid_atr > 0)
     {
      ...
      // double total_range = mid_h[idx_cl_mid] - mid_l[idx_cl_mid]; // Kept if needed later
      ...
      bool low_result  = (body < (0.35 * mid_atr));

      data.absorption = "NO"; // Default

      if(high_effort && low_result)
        {
         // Logic Update: Use VELOCITY (Immediate Impulse) to determine direction
         // Why? Because structural absorption (M15) confirmed by speed (M5) is a cleaner signal.

         if(data.velocity > 0)
            data.absorption = "BULL_ABS"; // Volume halted price, now moving UP
         else
            if(data.velocity < 0)
               data.absorption = "BEAR_ABS"; // Volume halted price, now moving DOWN
            else
               data.absorption = "NEUT_ABS"; // Volume halted price, and it stalled (0 velocity)
        }
      else
         if(bar_rvol > 3.5 && body < (0.6 * mid_atr))
           {
            // Volume Climax: Extreme volume override

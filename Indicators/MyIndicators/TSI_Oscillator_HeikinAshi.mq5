...
#property description "TSI Oscillator on Heikin Ashi data"
...
#property indicator_label1  "HA_TSI_Osc"
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("HA_TSI_Osc(%d,%d,%d)", g_ExtSlowPeriod, g_ExtFastPeriod, g_ExtSignalPeriod));
...
//+------------------------------------------------------------------+
//| TSI Oscillator on Heikin Ashi calculation function.              |
//+------------------------------------------------------------------+
...
//--- STEP 6: Calculate final TSI value (internal buffer)
   double buffer_tsi[];
...
//--- STEP 7: Calculate the Signal Line (internal buffer)
...
//--- STEP 8: Calculate the final Oscillator value
   for(int i = signal_start_pos; i < rates_total; i++)
     {

//+------------------------------------------------------------------+
//|                                             StochRSI_Slow_Pro.mq5|
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "3.30" // Upgraded with dynamic high-performance Standard/MTF support
...
#include <MyIncludes\StochRSI_Slow_Calculator.mqh>
...
//--- 3. Initialize Physical StochRSI Calculator
   if(is_ha)
      g_calculator = new CStochRSI_Slow_Calculator_HA();
   else
      g_calculator = new CStochRSI_Slow_Calculator();

   if(CheckPointer(g_calculator) == POINTER_INVALID)
     {
      Print("Critical Error: Failed to allocate StochRSI Calculator memory.");
      return(INIT_FAILED);
     }

   if(!g_calculator.Init(InpRSIPeriod, InpKPeriod, InpSlowingPeriod, InpSlowingMAType, InpDPeriod, InpDMAType))
     {
      Print("Critical Error: Failed to initialize StochRSI Calculator.");
      return(INIT_FAILED);
     }

//--- 4. Dynamic Setup of Indicator Shortname and Plots
   string tf_str = g_is_mtf_mode ? (" " + EnumToString(g_calc_timeframe)) : "";
   string short_name = StringFormat("StochRSI Slow%s%s(%d, %d)",
                                    is_ha ? " HA" : "",
                                    tf_str,
                                    InpRSIPeriod,
                                    InpKPeriod);
...
//--- Drawing offset configuration
   int draw_begin_k = InpRSIPeriod + InpKPeriod + InpSlowingPeriod - 3;
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   int required_bars = InpRSIPeriod + InpKPeriod + InpSlowingPeriod + InpDPeriod + 5;
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      g_calculator.Calculate(g_htf_count, 0, h_open, h_high, h_low, h_close, price_type, h_res_k, h_res_d);
   ...
   if(live_idx >= required_bars)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol, g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_calculator.Calculate(g_htf_count, g_htf_count, h_open, h_high, h_low, h_close, price_type, h_res_k, h_res_d);
        }
     }
...
//--- Delegate asynchronous history checking and forced redraws to DataSync daemon using correct lookback period
   int required_bars = InpRSIPeriod + InpKPeriod + InpSlowingPeriod + InpDPeriod + 10;

   ...
   if(InpSourcePrice <= PRICE_HA_CLOSE)
     {
      ...
      IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("StochRSI Slow HA(%d,%d)", InpRSIPeriod, InpKPeriod));
     }
   else
     {
      g_calculator = new CStochRSI_Slow_Calculator();
      IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("StochRSI Slow(%d,%d)", InpRSIPeriod, InpKPeriod));
     }

   if(CheckPointer(g_calculator) == POINTER_INVALID || !g_calculator.Init(InpRSIPeriod, InpKPeriod, InpSlowingPeriod, InpSlowingMAType, InpDPeriod, InpDMAType))
     {
      Print("Failed to create or initialize StochRSI Slow Calculator object.");
      return(INIT_FAILED);
     }

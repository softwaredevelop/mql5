...
input ENUM_DMI_OSC_TYPE  InpOscType       = OSC_PDI_MINUS_NDI;   // Oscillator Formula (+DI - -DI)
input int                InpDMIPeriod     = 10;                  // DMI Smoothing Period
input int                InpFastKPeriod   = 10;                  // Stochastic %K Lookback Period
input int                InpSlowKPeriod   = 3;                   // Stochastic %K Slowing Period
input ENUM_MA_TYPE       InpStochMethod   = SMA;                 // Smoothing Method for %K
input int                InpSmoothPeriod  = 3;                   // Stochastic %D Period (Signal)
input ENUM_MA_TYPE       InpSignalMethod  = SMA;                 // Smoothing Method for %D
...
// Configure Horizontal Levels
   ...
   int warmup = InpDMIPeriod + InpFastKPeriod + InpSlowKPeriod + InpSmoothPeriod - 2;
...
// 4. Initialize Core Calculator Engine
   ...
   if(CheckPointer(g_calculator) == POINTER_INVALID ||
      !g_calculator.Init(InpDMIPeriod, InpFastKPeriod, InpSlowKPeriod, InpSmoothPeriod, InpStochMethod, InpSignalMethod, InpOscType))
     {
      Print("Critical Error: Failed to create or initialize DMI Stochastic Calculator.");
      return INIT_FAILED;
     }
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      // Compute HTF DMI Stochastic Values
      if(vol_limit > 0)

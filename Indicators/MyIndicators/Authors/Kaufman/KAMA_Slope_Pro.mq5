...
#property version   "1.10" // Clean Visual Release: Natural Zero Histogram Anchor
#property description "Slope derivative (1st derivative) of Perry Kaufman's Adaptive Moving Average."
#property description "Features native & MTF pipelines with a symmetrical 5-zone momentum color matrix."

#property indicator_separate_window
#property indicator_buffers 3
#property indicator_plots   2

//--- Plot 1: KAMA Slope (Color Histogram)
#property indicator_label1  "KAMA Slope"
...
#include <MyIncludes\KAMA_Slope_Calculator.mqh>
...
CKamaSlopeCalculator     *g_calculator = NULL;
...
// 3. Initialize Physical Calculators
   g_calculator = new CKamaSlopeCalculator();
   if(CheckPointer(g_calculator) == POINTER_INVALID || !g_calculator.Init(InpErPeriod, InpFastEmaPeriod, InpSlowEmaPeriod, InpSourcePrice))
     {
      Print("Critical Error: Failed to create or initialize KAMA Slope Calculator.");
      return INIT_FAILED;
     }
...
// 4. Dynamic Setup of Indicator Shortname and Plots
   ...
   string short_name = StringFormat("KAMA Slope%s%s(%d,%d,%d)%s",
                                    ha_tag, tf_str,
...
// Drawing offset configuration
   int draw_begin = InpErPeriod + InpSignalPeriod + 5;
   if(g_is_mtf_mode)
      draw_begin = 0;
   ...
   if(!g_is_mtf_mode)
     {
      ...
      if(InpShowSignal)
         g_ma_calc.CalculateOnArray(rates_total, prev_calculated, BufferSlope, g_double_volume, BufferSignalMA, InpErPeriod + 1);
      else
        {
         for(int i = start_sync; i < rates_total; i++)
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      // Compute HTF KAMA Slope & Signal MA
      g_calculator.Calculate(g_htf_count, 0, h_open, h_high, h_low, h_close, h_res_slope, h_res_color, InpThreshold);
      g_ma_calc.CalculateOnArray(g_htf_count, 0, h_res_slope, h_volume, h_res_signal, InpErPeriod + 1);
   ...
   if(live_idx >= required_bars)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol, g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_calculator.Calculate(g_htf_count, g_htf_count, h_open, h_high, h_low, h_close, h_res_slope, h_res_color, InpThreshold);
         g_ma_calc.CalculateOnArray(g_htf_count, g_htf_count, h_res_slope, h_volume, h_res_signal, InpErPeriod + 1);
        }
     }

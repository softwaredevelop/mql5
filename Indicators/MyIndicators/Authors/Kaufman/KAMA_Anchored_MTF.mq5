//+------------------------------------------------------------------+
//|                                        KAMA_Anchored_MTF_Pro.mq5 |
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.10" // Upgraded with dynamic odd/even gapped segment drawing on HTF
#property description "Multi-Timeframe (MTF) Kaufman's Adaptive Moving Average with dynamic Anchored Resets."
#property description "Displays HTF Anchored KAMA cleanly directly on lower TF charts without live-bar warping or connecting line drag."
...
#property indicator_label2  "KAMA MTF (Segment)"
...
//--- Indicator Buffers ---
double    BufferKAMA_Odd_MTF[];
double    BufferKAMA_Even_MTF[];
...
//--- 2. Setup Buffers
   SetIndexBuffer(0, BufferKAMA_Odd_MTF,  INDICATOR_DATA);
   SetIndexBuffer(1, BufferKAMA_Even_MTF, INDICATOR_DATA);
   ArraySetAsSeries(BufferKAMA_Odd_MTF,  false); // Standard indexing for main buffer
   ArraySetAsSeries(BufferKAMA_Even_MTF, false); // Standard indexing for main buffer
...
//--- 3. Initialize Calculator
   ...
   if(CheckPointer(g_calculator) == POINTER_INVALID ||
     ...
     {
      Print("Failed to initialize KAMA Anchored Calculator object.");
      return(INIT_FAILED);
     }
...
//--- 4. Set Shortname
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("KAMA Anch%s%s(%s,%d)", type, tf_str, StringSubstr(anchor_name, 7), InpErPeriod));
   IndicatorSetInteger(INDICATOR_DIGITS, _Digits);

// Draw begin logic
   int draw_begin = InpErPeriod;
   if(g_is_mtf_mode)
      draw_begin = 0;
   ...
   if(!g_is_mtf_mode)
     {
      ...
      g_calculator.Calculate(rates_total, prev_calculated, price_type, time, open, high, low, close, BufferKAMA_Odd_MTF, BufferKAMA_Even_MTF);
      return(rates_total);
     }
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      //--- Calculate KAMA on HTF (Closed bars and forming bar initialized)
      g_calculator.Calculate(g_htf_count, 0, price_type, h_time, h_open, h_high, h_low, h_close, h_res_odd, h_res_even);
   ...
   if(live_idx >= InpErPeriod)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol,  g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_calculator.Calculate(g_htf_count, g_htf_count, price_type, h_time, h_open, h_high, h_low, h_close, h_res_odd, h_res_even);
        }
     }
      ...
      if(shift_htf >= 0)
        {
         ...
         if(idx_htf >= 0 && idx_htf < g_htf_count)
           {
            BufferKAMA_Odd_MTF[i]  = h_res_odd[idx_htf];
            BufferKAMA_Even_MTF[i] = h_res_even[idx_htf];
           }
         else
           {
            BufferKAMA_Odd_MTF[i]  = EMPTY_VALUE;
            BufferKAMA_Even_MTF[i] = EMPTY_VALUE;
           }
        }
      else
        {
...
void OnTimer()
  {
   if(!g_data_synced)
     {
      int required_bars = InpErPeriod + 5;
      if(EnsureHTFDataReady(_Symbol, g_calc_timeframe, required_bars))
        {
         g_data_synced = true;

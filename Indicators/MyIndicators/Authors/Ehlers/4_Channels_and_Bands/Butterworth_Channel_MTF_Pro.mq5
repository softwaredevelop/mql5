//+------------------------------------------------------------------+
//|                                   Butterworth_Channel_MTF_Pro.mq5|
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // Dynamic Multi-Timeframe Butterworth Channel with flat-force step-alignment
#property description "Multi-Timeframe (MTF) John Ehlers' Butterworth Channel (Keltner Concept)."
#property description "Displays HTF Butterworth baseline and ATR bands cleanly on current chart without live-bar warping."
...
//--- 3. Initialize Calculator
   ...
   if(CheckPointer(g_calculator) == POINTER_INVALID ||
     ...
     {
      Print("Failed to create or initialize Butterworth Channel Calculator object.");
      return(INIT_FAILED);
     }
...
//--- 4. Set Shortname
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("Butterworth Ch%s%s(%d,%d, ATR %d)", type, tf_str, InpPeriod, (int)InpPoles, InpAtrPeriod));
...
//--- Ensure target timeframe history is ready
   int required_bars = InpPeriod + InpAtrPeriod + 10;
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      //--- Calculate Butterworth Channel on HTF (Closed bars and forming bar initialized)
...
void OnTimer()
  {
   if(!g_data_synced)
     {
      int required_bars = InpPeriod + InpAtrPeriod + 5;
      if(EnsureHTFDataReady(_Symbol, g_calc_timeframe, required_bars))
        {
         g_data_synced = true;

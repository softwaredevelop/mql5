//+------------------------------------------------------------------+
//|                                      Laguerre_Bands_MTF_Pro.mq5  |
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // Dynamic Multi-Timeframe Laguerre Bands with flat-force step-alignment
#property description "Multi-Timeframe (MTF) John Ehlers' Laguerre Bands (Bollinger Concept)."
#property description "Displays HTF Laguerre baseline and StdDev bands cleanly on current chart without live-bar warping."
...
#property indicator_label3  "Laguerre MTF"
...
#include <MyIncludes\Laguerre_Bands_Calculator.mqh>
...
input int                       InpPeriod         = 20;            // Standard Deviation Period
...
//--- Global variables ---
CLaguerreBandsCalculator *g_calculator;
...
//--- 3. Initialize Calculator
   if(InpSourcePrice <= PRICE_HA_CLOSE)
      g_calculator = new CLaguerreBandsCalculator_HA();
   else
      g_calculator = new CLaguerreBandsCalculator();

   if(CheckPointer(g_calculator) == POINTER_INVALID ||
      !g_calculator.Init(InpGamma, InpPeriod, InpDeviation))
     {
      Print("Failed to create or initialize Laguerre Bands Calculator object.");
      return(INIT_FAILED);
     }
...
//--- 4. Set Shortname - Updated format string to %.3f to support exact Fibonacci decimals
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("Laguerre Bands%s%s(%.3f, %d, %.1f)", type, tf_str, InpGamma, InpPeriod, InpDeviation));
   ...
   if(!g_is_mtf_mode)
     {
      g_calculator.Calculate(rates_total, prev_calculated, open, high, low, close, price_type,
                             BufferMiddle_MTF, BufferUpper_MTF, BufferLower_MTF);
      return(rates_total);
     }
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      //--- Calculate Laguerre Bands on HTF (Closed bars and forming bar initialized)
      g_calculator.Calculate(g_htf_count, 0, h_open, h_high, h_low, h_close, price_type,
                             h_res_mid, h_res_up, h_res_lo);
   ...
   if(live_idx >= InpPeriod)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol, g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_calculator.Calculate(g_htf_count, g_htf_count, h_open, h_high, h_low, h_close, price_type,
                                h_res_mid, h_res_up, h_res_lo);
        }
     }

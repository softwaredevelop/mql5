//+------------------------------------------------------------------+
//|                                      Butterworth_Bands_MTF_Pro.mq5|
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // Dynamic Multi-Timeframe Butterworth Bands with flat-force step-alignment
#property description "Multi-Timeframe (MTF) John Ehlers' Butterworth Bands (Bollinger Concept)."
#property description "Displays HTF Butterworth baseline and StdDev bands cleanly on current chart without live-bar warping."
...
//--- Levels (Bollinger Standard Constant Boundaries)
#property indicator_level1 0.0
#property indicator_levelstyle STYLE_DOT
#property indicator_levelcolor clrSilver

//--- Include the calculator engine ---
#include <MyIncludes\Butterworth_Bands_Calculator.mqh>
...
input int                       InpButterPeriod = 20;              // Filter Period
...
//|                                                                  |
//+------------------------------------------------------------------+
input group "Bands (StdDev) Settings"
input int                       InpPeriod       = 20;              // Standard Deviation Period (N)
input double                    InpDeviation    = 2.0;             // Deviation Multiplier (d)

//--- Indicator Buffers ---
double    BufferUpper_MTF[];
double    BufferLower_MTF[];
double    BufferMiddle_MTF[];
...
//--- Global variables ---
CButterworthBandsCalculator *g_calculator;
...
//--- 2. Setup Buffers
   SetIndexBuffer(0, BufferUpper_MTF,  INDICATOR_DATA);
   SetIndexBuffer(1, BufferLower_MTF,  INDICATOR_DATA);
   SetIndexBuffer(2, BufferMiddle_MTF, INDICATOR_DATA);
   ArraySetAsSeries(BufferUpper_MTF,  false);
   ArraySetAsSeries(BufferLower_MTF,  false);
   ArraySetAsSeries(BufferMiddle_MTF, false);
...
//--- 3. Initialize Calculator
   if(InpSourcePrice <= PRICE_HA_CLOSE)
      g_calculator = new CButterworthBandsCalculator_HA();
   else
      g_calculator = new CButterworthBandsCalculator();

   if(CheckPointer(g_calculator) == POINTER_INVALID ||
      !g_calculator.Init(InpPeriod, InpDeviation, InpButterPeriod, InpPoles))
     {
      Print("Failed to create or initialize Butterworth Bands Calculator object.");
      return(INIT_FAILED);
     }

//--- 4. Set Shortname - Dynamic 3-digit Gamma precision (%.3f)
   string type = (InpSourcePrice <= PRICE_HA_CLOSE) ? " HA" : "";
   string tf_str = g_is_mtf_mode ? (" " + EnumToString(g_calc_timeframe)) : "";
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("Butterworth Bands%s%s(%d,%d,%.1f)", type, tf_str, InpButterPeriod, InpPeriod, InpDeviation));
...
// Draw begin logic
   ...
   PlotIndexSetInteger(2, PLOT_DRAW_BEGIN, g_is_mtf_mode ? 0 : 2);
...
//================================================================
   if(!g_is_mtf_mode)
     {
      g_calculator.Calculate(rates_total, prev_calculated, open, high, low, close, price_type, BufferMiddle_MTF, BufferUpper_MTF, BufferLower_MTF);
      return(rates_total);
     }
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      //--- Calculate Butterworth Bands on HTF (Closed bars and forming bar initialized)
      g_calculator.Calculate(g_htf_count, 0, h_open, h_high, h_low, h_close, price_type, h_res_mid, h_res_up, h_res_lo);
   ...
   if(live_idx >= InpPeriod)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol, g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_calculator.Calculate(g_htf_count, g_htf_count, h_open, h_high, h_low, h_close, price_type, h_res_mid, h_res_up, h_res_lo);
        }
     }
      ...
      if(shift_htf >= 0)
        {
         ...
         if(idx_htf >= 0 && idx_htf < g_htf_count)
           {
            BufferMiddle_MTF[i] = h_res_mid[idx_htf];
            BufferUpper_MTF[i]  = h_res_up[idx_htf];
            BufferLower_MTF[i]  = h_res_lo[idx_htf];
           }
         else
           {
            BufferMiddle_MTF[i] = EMPTY_VALUE;
            BufferUpper_MTF[i]  = EMPTY_VALUE;
            BufferLower_MTF[i]  = EMPTY_VALUE;
           }
        }
      else
        {

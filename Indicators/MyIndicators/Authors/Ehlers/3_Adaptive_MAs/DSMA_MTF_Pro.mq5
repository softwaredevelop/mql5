...
#property indicator_label1  "DSMA MTF"
...
input group "DSMA Settings"
input int                       InpPeriod         = 40;            // Critical Period
input ENUM_APPLIED_PRICE_HA_ALL InpSourcePrice    = PRICE_CLOSE_STD; // Price Source

//--- Indicator Buffers ---
double    BufferDSMA_MTF[];
...
//--- 2. Setup Buffers
   SetIndexBuffer(0, BufferDSMA_MTF, INDICATOR_DATA);
   ArraySetAsSeries(BufferDSMA_MTF, false); // Standard indexing
...
//--- 4. Set Shortname
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("DSMA%s%s(%d)", type, tf_str, InpPeriod));

// Draw begin logic (approximate for MTF)
   int draw_begin = InpPeriod + 2;
   if(g_is_mtf_mode)
      draw_begin = 0; // In MTF mode, we map what we have
...
//================================================================
   if(!g_is_mtf_mode)
     {
      g_calculator.Calculate(rates_total, prev_calculated, price_type, open, high, low, close, BufferDSMA_MTF);
      return(rates_total);
     }
   ...
   if(prev_calculated == 0)
     {
      ...
      ArrayInitialize(BufferDSMA_MTF, EMPTY_VALUE);
     }
...
//--- E. Calculate HTF DSMA (Incremental)
   ...
   for(int i = limit; i < rates_total; i++)
     {
      ...
      if(htf_index >= 0 && htf_index < htf_rates_total)
        {
         BufferDSMA_MTF[i] = g_htf_buffer[htf_index];
        }
      else
        {
         BufferDSMA_MTF[i] = EMPTY_VALUE;
        }
     }

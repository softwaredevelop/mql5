//+------------------------------------------------------------------+
//|                                       Laguerre_Slope_MTF_Pro.mq5|
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // High-performance MTF with staircase synchronization
#property description "Multi-Timeframe Slope derivative of John Ehlers' Laguerre Filter."
#property description "Features non-warping step synchronization and 5-zone symmetrical thermal coloring."
...
#property indicator_label1  "Laguerre Slope MTF"
...
#property indicator_label2  "Signal MA MTF"
...
#include <MyIncludes\DataSync_Tools.mqh> // Centralized multi-timeframe synchronization tools
...
//--- 3. Initialize Physical Laguerre Calculator
   ...
   if(CheckPointer(g_calculator) == POINTER_INVALID || !g_calculator.Init(InpGamma, SOURCE_PRICE, is_ha))
     {
      Print("Critical Error: Failed to create or initialize Laguerre Slope Calculator.");
      return(INIT_FAILED);
     }
...
//--- 5. Dynamic Setup of Indicator Shortname and Plots
   ...
   string short_name = StringFormat("Laguerre Slope%s%s(%.3f, %.5f)%s",
                                    is_ha ? " HA" : "",
                                    tf_str,
                                    InpGamma,
                                    InpThreshold,
...
//--- Drawing offset configuration
   int draw_begin = InpSignalPeriod + 5;
   if(g_is_mtf_mode)
      draw_begin = 0; // Handled by mapped buffers

   PlotIndexSetInteger(0, PLOT_DRAW_BEGIN, draw_begin);
   PlotIndexSetInteger(1, PLOT_DRAW_BEGIN, draw_begin);
   IndicatorSetInteger(INDICATOR_DIGITS, _Digits + 2);

//--- 6. Initialize Background Synchronization Timer Daemon
   if(g_is_mtf_mode)
      EventSetTimer(1);
...
//===================================================================
// MODE 2: Multi-Timeframe Engine (Warp-free synchronization)
//===================================================================
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      g_htf_count = MathMin(htf_bars, 3000); // Guard rails to prevent thread bloat
      ...
      g_ma_calc.CalculateOnArray(g_htf_count, 0, h_res_slope, h_volume, h_res_signal, 1);
   ...
   if(live_idx >= 10)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol, g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_ma_calc.CalculateOnArray(g_htf_count, g_htf_count, h_res_slope, h_volume, h_res_signal, 1);
        }
     }

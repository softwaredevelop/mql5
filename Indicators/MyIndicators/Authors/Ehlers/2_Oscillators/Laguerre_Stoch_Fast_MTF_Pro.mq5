//+------------------------------------------------------------------+
//|                                  Laguerre_Stoch_Fast_MTF_Pro.mq5 |
...
input ENUM_MA_TYPE              InpSignalMethod   = SMA;           // Signal Line MA Type

//--- Buffers
double    BufferStoch_MTF[];
double    BufferSignal_MTF[];

//--- Internal HTF Data Caches
double    h_res_stoch[]; // HTF Stochastic Results cached
...
//--- 2. Setup Buffers
   SetIndexBuffer(0, BufferStoch_MTF,   INDICATOR_DATA);
   SetIndexBuffer(1, BufferSignal_MTF,  INDICATOR_DATA);
   ArraySetAsSeries(BufferStoch_MTF,   false);
...
//--- 3. Initialize Calculator
   ...
   if(CheckPointer(g_calculator) == POINTER_INVALID || !g_calculator.Init(InpGamma, InpSignalPeriod, InpSignalMethod))
     {
      Print("Failed to create or initialize Laguerre Stoch Fast Calculator object.");
      return(INIT_FAILED);
     }
...
// Draw begin logic
   ...
   PlotIndexSetInteger(1, PLOT_DRAW_BEGIN, draw_begin + InpSignalPeriod);
   ...
   if(!g_is_mtf_mode)
     {
      ...
      if(volume_limit > 0)
         g_calculator.Calculate(rates_total, prev_calculated, price_type, open, high, low, close, volume, BufferStoch_MTF, BufferSignal_MTF);
      else
         g_calculator.Calculate(rates_total, prev_calculated, price_type, open, high, low, close, tick_volume, BufferStoch_MTF, BufferSignal_MTF);
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      //--- Calculate Laguerre Stochastic Fast and Signal on HTF (Closed bars and forming bar initialized)
      g_calculator.Calculate(g_htf_count, 0, price_type, h_open, h_high, h_low, h_close, h_vol, h_res_stoch, h_res_sig);
   ...
   if(live_idx >= 2)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol, g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_calculator.Calculate(g_htf_count, g_htf_count, price_type, h_open, h_high, h_low, h_close, h_vol, h_res_stoch, h_res_sig);
        }
     }
      ...
      if(shift_htf >= 0)
        {
         ...
         if(idx_htf >= 0 && idx_htf < g_htf_count)
           {
            BufferStoch_MTF[i]  = h_res_stoch[idx_htf];
            BufferSignal_MTF[i] = h_res_sig[idx_htf];
           }
         else
           {
            BufferStoch_MTF[i]  = EMPTY_VALUE;

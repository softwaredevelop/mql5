//+------------------------------------------------------------------+
//|                            StochasticSlow_on_LaguerreRSI_MTF_Pro.mq5 |
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // Dynamic Multi-Timeframe Slow Stochastic on Laguerre RSI with flat-force step-alignment
#property description "Multi-Timeframe (MTF) Slow Stochastic on Laguerre RSI."
#property description "Displays HTF Slow Stochastic on Laguerre RSI cleanly without live-bar warping."
...
//--- 3. Initialize Calculator
   ...
   if(CheckPointer(g_calculator) == POINTER_INVALID ||
      !g_calculator.Init(InpGamma, InpKPeriod, InpSlowingPeriod, InpSlowingMethod, InpDPeriod, InpDMAType))
     {
...
//--- 4. Set Shortname
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("StochSlow on LagRSI%s%s(%.2f, %d, %d)", type, tf_str, InpGamma, InpKPeriod, InpSlowingPeriod));
...
//--- Ensure target timeframe history is ready
   int required_bars = InpKPeriod + InpSlowingPeriod + 20;
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      //--- Calculate Slow Stochastic on HTF (Closed bars and forming bar initialized)
...
void OnTimer()
  {
   if(!g_data_synced)
     {
      int required_bars = InpKPeriod + InpSlowingPeriod + 5;
      if(EnsureHTFDataReady(_Symbol, g_calc_timeframe, required_bars))
        {
         g_data_synced = true;

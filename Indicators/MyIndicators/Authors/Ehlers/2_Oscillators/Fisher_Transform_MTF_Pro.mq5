//+------------------------------------------------------------------+
//|                                        Fisher_Transform_MTF_Pro.mq5 |
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // Dynamic Multi-Timeframe Fisher Transform with flat-force step-alignment
#property description "Multi-Timeframe (MTF) John Ehlers' Fisher Transform."
#property description "Displays HTF Fisher and Signal lines cleanly on current chart without live-bar warping."
...
//--- Plot 1: Fisher line
#property indicator_label1  "Fisher MTF"
...
//--- Buffers
double    BufferFisher_MTF[];
double    BufferSignal_MTF[];

//--- Internal HTF Data Caches
double    h_res_fish[];  // HTF Fisher Results cached
...
//--- 2. Setup Buffers
   SetIndexBuffer(0, BufferFisher_MTF, INDICATOR_DATA);
   SetIndexBuffer(1, BufferSignal_MTF, INDICATOR_DATA);
   ArraySetAsSeries(BufferFisher_MTF, false);
...
//--- 4. Set Shortname
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("Fisher Pro%s%s(%d,%.2f,%s)", type, tf_str, InpPeriod, InpAlpha, sigStr));
   ...
   if(!g_is_mtf_mode)
     {
      ...
      if(volume_limit > 0)
         g_calculator.Calculate(rates_total, prev_calculated, open, high, low, close, volume, BufferFisher_MTF, BufferSignal_MTF);
      else
         g_calculator.Calculate(rates_total, prev_calculated, open, high, low, close, tick_volume, BufferFisher_MTF, BufferSignal_MTF);
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      ArrayResize(h_res_fish, g_htf_count);
      ...
      //--- Calculate Fisher on HTF (Closed bars and forming bar initialized)
      g_calculator.Calculate(g_htf_count, 0, h_open, h_high, h_low, h_close, h_vol, h_res_fish, h_res_sig);
   ...
   if(live_idx >= InpPeriod)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol, g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_calculator.Calculate(g_htf_count, g_htf_count, h_open, h_high, h_low, h_close, h_vol, h_res_fish, h_res_sig);
        }
     }
      ...
      if(shift_htf >= 0)
        {
         ...
         if(idx_htf >= 0 && idx_htf < g_htf_count)
           {
            BufferFisher_MTF[i] = h_res_fish[idx_htf];
            BufferSignal_MTF[i] = h_res_sig[idx_htf];
           }
         else
           {
            BufferFisher_MTF[i] = EMPTY_VALUE;

//+------------------------------------------------------------------+
//|                                 Ehlers_Smoother_Acceleration_Pro.mq5|
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // First unified Standard & MTF Ehlers Smoother Acceleration release
#property description "Acceleration (2nd derivative) of John Ehlers' SuperSmoother and UltimateSmoother."
#property description "Features standard & MTF support with a 5-zone thermal color palette."

#property indicator_separate_window
#property indicator_buffers 3
#property indicator_plots   2

//--- Plot 1: Smoother Acceleration (Color Histogram)
#property indicator_label1  "Smoother Accel"
...
#include <MyIncludes\Ehlers_Smoother_Acceleration_Calculator.mqh>
...
CEhlersSmootherAccelerationCalculator *g_calculator;
...
//--- 3. Initialize Physical Acceleration Calculator
   g_calculator = new CEhlersSmootherAccelerationCalculator();
   if(CheckPointer(g_calculator) == POINTER_INVALID || !g_calculator.Init(InpPeriod, InpSmootherType, is_ha))
     {
      Print("Critical Error: Failed to create or initialize Ehlers Smoother Acceleration Calculator.");
      return(INIT_FAILED);
     }
...
//--- 5. Dynamic Setup of Indicator Shortname and Plots
   ...
   string type_name = (InpSmootherType == SUPERSMOOTHER) ? "Super" : "Ultimate";
   string tf_str = g_is_mtf_mode ? (" " + EnumToString(g_calc_timeframe)) : "";
   string short_name = StringFormat("%sSmoother Accel%s%s(%d)%s",
                                    type_name,
                                    is_ha ? " HA" : "",
                                    tf_str,
                                    InpPeriod,
...
//--- Drawing offset configuration
   int draw_begin = InpPeriod + InpSignalPeriod + 6;
   if(g_is_mtf_mode)
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      //--- Calculate HTF core Ehlers Smoother Acceleration

...
#property version   "3.20" // Fixed: Leak-Free Persistent State & Clean Scaling
#property description "Professional E-Score (Ehlers Smoother Z-Score in Sigma units)."
#property description "Measures standardized statistical price deviations from John Ehlers' Low-Pass DSP Filters."
...
input group "--- E-Score Settings ---"
input ENUM_SMOOTHER_TYPE        InpSmootherType   = SUPERSMOOTHER;       // Underlying Smoother (SuperSmoother / UltimateSmoother)
...
double    h_res_escore[], h_res_color[], h_res_signal[];
datetime  h_time[];

//--- Global Objects & State Management
CEScoreCalculator        *g_calc = NULL;
...
// 2. Bind Buffers
   ...
   ArrayInitialize(ExtEScoreBuffer, 0.0);
...
// 4. Initialize Core E-Score Calculator
   g_calc = new CEScoreCalculator();
   if(CheckPointer(g_calc) == POINTER_INVALID || !g_calc.Init(InpPeriod, InpSmootherType, InpSourcePrice))
     {
      Print("Critical Error: Failed to initialize EScore Calculator Engine.");
      return INIT_FAILED;
     }
...
// 6. Dynamic Indicator Shortname Setup
   ...
   string short_name = StringFormat("E-Score%s%s(%s, %d)%s",
                                    ha_tag, tf_str,
                                    name, InpPeriod,
                                    sig_str);
   ...
   if(!g_is_mtf_mode)
     {
      ...
      // 1. Calculate E-Score
      g_calc.Calculate(rates_total, prev_calculated, open, high, low, close, ExtEScoreBuffer);

      // 2. Calculate Signal MA Line
      if(InpShowSignal && CheckPointer(g_signal_calculator) != POINTER_INVALID)
        {
         g_signal_calculator.CalculateOnArray(rates_total, prev_calculated, ExtEScoreBuffer, g_double_volume, ExtSignalBuffer, warmup);
        }
      else
        {
         for(int i = start_sync; i < rates_total; i++)
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      ArraySetAsSeries(h_res_escore, false);
      ...
      // Compute HTF E-Score Values
      g_calc.Calculate(g_htf_count, 0, h_open, h_high, h_low, h_close, h_res_escore);
      ...
      if(InpShowSignal && CheckPointer(g_signal_calculator) != POINTER_INVALID)
        {
         ...
         g_signal_calculator.CalculateOnArray(g_htf_count, 0, h_res_escore, htf_double_vol, h_res_signal, warmup);
        }

      // Color HTF Cache
      for(int i = 0; i < g_htf_count; i++)
        {
         double v = h_res_escore[i];
         if(v >= InpLevelClimaxHigh)
            h_res_color[i] = 2.0;
         else
            if(v >= InpLevelFlowHigh)
   ...
   if(live_idx >= required_bars)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyTickVolume(_Symbol, g_calc_timeframe, shift, 1, tv)    == 1)
        {
         ...
         g_calc.Calculate(g_htf_count, g_htf_count, h_open, h_high, h_low, h_close, h_res_escore);
         ...
         if(InpShowSignal && CheckPointer(g_signal_calculator) != POINTER_INVALID)
           {
            ...
            g_signal_calculator.CalculateOnArray(g_htf_count, g_htf_count, h_res_escore, htf_double_vol, h_res_signal, warmup);
           }

         double v_val = h_res_escore[live_idx];

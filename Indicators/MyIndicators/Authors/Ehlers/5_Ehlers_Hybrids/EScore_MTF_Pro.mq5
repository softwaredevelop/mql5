...
#property version   "1.00" // Fully functional dynamic Multi-Timeframe E-Score (Ehlers Smoother Z-Score)
#property description "Professional Multi-Timeframe (MTF) E-Score Oscillator with dynamic Signal Line."
#property description "Displays HTF Ehlers Z-Score cleanly directly on lower TF charts without live-bar warping."
...
#property indicator_label1  "E-Score MTF"
...
//--- Buffers
double    BufferEScore_MTF[];
double    BufferColors_MTF[];
double    BufferSignal_MTF[];

//--- Internal HTF Data Caches
double    h_res_escore[]; // HTF EScore Results cached
...
CEScoreCalculator        *g_calculator;
...
//--- 4. Initialize Calculator
   ...
   g_calculator = new CEScoreCalculator();
   if(CheckPointer(g_calculator) == POINTER_INVALID || !g_calculator.Init(InpPeriod, InpSmootherType, use_ha))
     {
      Print("Failed to create or initialize E-Score Calculator.");
      return(INIT_FAILED);
     }
   ...
   if(InpShowSignal)
     {
      ...
      short_name = StringFormat("E-Score%s%s(%d) %s(%d)", (use_ha ? " HA" : ""), tf_str, InpPeriod, sig_name, InpSignalPeriod);
     }
   else
     {
      short_name = StringFormat("E-Score%s%s(%d)", (use_ha ? " HA" : ""), tf_str, InpPeriod);
     }

   IndicatorSetString(INDICATOR_SHORTNAME, short_name);
   PlotIndexSetString(0, PLOT_LABEL, "E-Score MTF");
   ...
   if(!g_is_mtf_mode)
     {
      ...
      // Calculate E-Score
      g_calculator.Calculate(rates_total, prev_calculated, price_type, open, high, low, close, BufferEScore_MTF);

      // Calculate Signal Line
      if(InpShowSignal && CheckPointer(g_signal_calculator) != POINTER_INVALID)
        {
         g_signal_calculator.CalculateOnArray(rates_total, prev_calculated, BufferEScore_MTF, g_double_volume, BufferSignal_MTF, InpPeriod);
        }
   ...
   if(htf_updated || prev_calculated == 0)
     {
      ...
      ArrayResize(h_res_escore, g_htf_count);
      ...
      //--- Calculate EScore on HTF (Closed bars and forming bar initialized)
      g_calculator.Calculate(g_htf_count, 0, price_type, h_open, h_high, h_low, h_close, h_res_escore);

      //--- Calculate HTF Signal Line
      if(InpShowSignal && CheckPointer(g_signal_calculator) != POINTER_INVALID)
        {
         g_signal_calculator.CalculateOnArray(g_htf_count, 0, h_res_escore, h_vol_double, h_res_sig, InpPeriod);
        }
   ...
   if(live_idx >= InpPeriod + 5)
     {
      ...
      if(shift >= 0 &&
         ...
         CopyClose(_Symbol, g_calc_timeframe, shift, 1, c) == 1)
        {
         ...
         g_calculator.Calculate(g_htf_count, g_htf_count, price_type, h_open, h_high, h_low, h_close, h_res_escore);

         // Recalculate Signal Line on HTF live index
         if(InpShowSignal && CheckPointer(g_signal_calculator) != POINTER_INVALID)
           {
            g_signal_calculator.CalculateOnArray(g_htf_count, g_htf_count, h_res_escore, h_vol_double, h_res_sig, InpPeriod);
           }
        }
     }
      ...
      if(shift_htf >= 0)
        {
         ...
         if(idx_htf >= 0 && idx_htf < g_htf_count)
           {
            double v = h_res_escore[idx_htf];

            BufferEScore_MTF[i] = v;
           ...
           {
            BufferEScore_MTF[i] = EMPTY_VALUE;
            BufferSignal_MTF[i] = EMPTY_VALUE;
            BufferColors_MTF[i] = 0.0;
           }
        }

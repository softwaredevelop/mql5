...
#property description "Plots only the MACD Line. Designed for applying external"
#property description "moving averages for testing."
...
// Dummy signal params (9, SMA)
   if(CheckPointer(g_calculator) == POINTER_INVALID || !g_calculator.Init(InpFastPeriod, InpSlowPeriod, 9, InpSourceMAType, SMA))
     {
      Print("Failed to create or initialize MACD Calculator.");
      return(INIT_FAILED);
     }

   string short_name = StringFormat("MACD Line%s(%d,%d)", (InpSourcePrice <= PRICE_HA_CLOSE ? " HA" : ""), InpFastPeriod, InpSlowPeriod);
   IndicatorSetString(INDICATOR_SHORTNAME, short_name);

   PlotIndexSetInteger(0, PLOT_DRAW_BEGIN, MathMax(InpFastPeriod, InpSlowPeriod));

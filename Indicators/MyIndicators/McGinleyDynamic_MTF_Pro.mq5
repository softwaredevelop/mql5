//+------------------------------------------------------------------+
//|                                    McGinleyDynamic_MTF_Pro.mq5   |
//|                                          Copyright 2025, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2025, xxxxxxxx"
#property version   "3.30" // Unified MTF Engine Pattern
#property description "Multi-Timeframe (MTF) McGinley Dynamic Indicator."
...
#property indicator_label1  "McGinley MTF"
...
input group "McGinley Settings"
input int                       InpLength       = 14;              // Length
input ENUM_APPLIED_PRICE_HA_ALL InpSourcePrice  = PRICE_CLOSE_STD; // Price Source

//--- Indicator Buffers ---
double    BufferMcGinley_MTF[];
...
//--- 2. Setup Buffers
   SetIndexBuffer(0, BufferMcGinley_MTF, INDICATOR_DATA);
   ArraySetAsSeries(BufferMcGinley_MTF, false); // Standard indexing
...
//--- 3. Initialize Calculator
   ...
   if(CheckPointer(g_calculator) == POINTER_INVALID || !g_calculator.Init(InpLength))
     {
      Print("Failed to initialize McGinley Calculator.");
      return(INIT_FAILED);
     }
...
//--- 4. Set Shortname
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("McGinley%s%s(%d)", type, tf_str, InpLength));
...
//================================================================
   if(!g_is_mtf_mode)
     {
      g_calculator.Calculate(rates_total, prev_calculated, price_type, open, high, low, close, BufferMcGinley_MTF);
      return(rates_total);
     }
   ...
   if(prev_calculated == 0)
     {
      ...
      ArrayInitialize(BufferMcGinley_MTF, EMPTY_VALUE);
     }
...
//--- E. Calculate HTF McGinley (Incremental)
   ...
   for(int i = limit; i < rates_total; i++)
     {
      ...
      if(htf_index >= 0 && htf_index < htf_rates_total)
        {
         BufferMcGinley_MTF[i] = g_htf_buffer[htf_index];
        }
      else
        {
         BufferMcGinley_MTF[i] = EMPTY_VALUE;
        }
     }

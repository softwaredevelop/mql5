//+------------------------------------------------------------------+
//|                                        LinReg_R2_Dashboard_Pro.mq5 |
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // Fully functional dynamic Multi-Asset LinReg R2 Thermal Scanner
#property description "Minimalist and Live-Updating LinReg R2 Multi-Asset Thermal Scanner."
#property description "Companion dashboard for LinReg_R2_Pro. Displays R2 Trend Quality and Slope Direction."
...
//| RenderCell                                                       |
//+------------------------------------------------------------------+
void RenderCell(string symbol, double r2_val, double slope_val, int x, int y, int w, int h)
  {
...
//--- 1. Render Table Header
   ...
   CreateButton(g_prefix + "H_Val", "R2 & Slope", InpTableX + col_w_sym + 2, header_y, col_w_val, row_h, clrDarkSlateGray, clrWhite);
   ...
   for(int r = 0; r < g_symbols_total; r++)
     {
      ...
      // Get exact real-time Linear Regression R2 and Slope
      double r2_val = EMPTY_VALUE;
      double slope_val = 0.0;

      GetLinRegValues(sym, InpTimeframe, InpPeriod, r2_val, slope_val);

      // Render the thermal cell
      RenderCell(sym, r2_val, slope_val, InpTableX + col_w_sym + 2, row_y, col_w_val, row_h);
     }
...
int OnInit()
  {
   g_updating = false;
   g_last_update_ms = 0;
   g_prefix = StringFormat("LRR2D_%I64d_", ChartID()); // Cleaned with dynamic LRR2D prefix

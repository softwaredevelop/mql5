//+------------------------------------------------------------------+
//|                               DMIStochastic_Adaptive_MTF_Pro.mq5 |
//|                                          Copyright 2026, xxxxxxxx|
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, xxxxxxxx"
#property version   "1.00" // Dynamic Multi-Timeframe Adaptive DMI Stochastic with flat-force step-alignment
#property description "Barbara Star's Adaptive DMI Stochastic Oscillator (Multi-Timeframe)."
#property description "Displays HTF Adaptive DMI Stochastic %K and %D lines cleanly without live-bar warping."
...
//--- 3. Initialize Calculator (Factory Logic)
   ...
   if(CheckPointer(g_calculator) == POINTER_INVALID ||
     ...
     {
      Print("Failed to create or initialize DMI Adaptive Stochastic Calculator object.");
      return(INIT_FAILED);
     }
...
//--- 4. Set Shortname
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, StringFormat("DMI Stoch Adapt%s%s(%d,%d,%d-%d)", type, tf_str, InpDMIPeriod, InpErPeriod, InpMinStochPeriod, InpMaxStochPeriod));
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   ENUM_APPLIED_PRICE price_type = PRICE_CLOSE; // Classic close mapping for internal DMI
...
//--- Ensure target timeframe history is ready
   int required_bars = InpDMIPeriod + MathMax(InpErPeriod, InpMaxStochPeriod) + InpSlowingPeriod + InpDPeriod + 10;
...
//--- 2. Live Update for the Current Forming HTF Bar (Index: g_htf_count - 1) on every tick!
   ...
   if(live_idx >= InpDMIPeriod + MathMax(InpErPeriod, InpMaxStochPeriod))
     {
...
void OnTimer()
  {
   if(!g_data_synced)
     {
      int required_bars = InpDMIPeriod + MathMax(InpErPeriod, InpMaxStochPeriod) + InpSlowingPeriod + InpDPeriod + 5;
      if(EnsureHTFDataReady(_Symbol, g_calc_timeframe, required_bars))
        {
         g_data_synced = true;
